...
def cleanup(**kwargs):
    ...
    accounts = settings.gce
    for account in accounts:
        client = wrapanapi.GoogleCloudSystem(
            project=account.auth.project_id,
            service_account=json.loads(account.auth.service_account),
        )
        logger.info(f"\nExecution for account: {account['NAME']}")
        with compute_client("gce", client=client) as gce_client:
            ...
            if kwargs["discs"] or kwargs["_all"]:
                logger.warning(
                    "Cloudwash dependency 'WrapanAPI' does not supports DISCs operation for GCE yet!"  # noqa
                )
            if is_dry_run:
                echo_dry(dry_data)
